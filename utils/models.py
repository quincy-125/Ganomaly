...
def build_generator(z_dim, epsilon=1e-8):
    ...
    :return: generator Model of size (None, 3, 4, 4) # Channels first
    """
    ...
    model = tf.keras.layers.Conv2D(512, 3, padding='same', data_format='channels_first')(model)
    model = pixel_norm(model, epsilon=epsilon)
    model = tf.keras.layers.LeakyReLU(alpha=0.2)(model)

    # To RGB
    model = tf.keras.layers.Conv2D(3, 3, padding='same', data_format='channels_first')(model)
